<?php
	...
	function select6LatestBook($conn){
		$row = array();
	...
	function insertIntoOrder($conn, $total_price, $date, $name, $address, $city, $phone_number){
		$query = "INSERT INTO orders VALUES 
		('','" . $total_price . "', '" . $date . "', '" . $name . "', '" . $address . "', '" . $city . "', '" . $phone_number . "')";
		$result = mysqli_query($conn, $query);
		if(!$result){
	...
	function getWriterName($conn, $pubid){
		$query = "SELECT writer_name FROM writer WHERE writerid = '$writerid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
